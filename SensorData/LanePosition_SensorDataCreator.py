...
def write_scenarios():
    ...
    create_patterned_file('Senario1/LanePosSensorData.csv', ['N'])

    # 2. N, W, C (C grouped at end)
    create_patterned_file('Senario2/LanePosSensorData.csv', ['N', 'W', 'C'])

    # 3. N, W, C, F (C then F at end)
    create_patterned_file('Senario3/LanePosSensorData.csv', ['N', 'W', 'C', 'F'])

    print("three CSV files created for Lane Position sensor.")